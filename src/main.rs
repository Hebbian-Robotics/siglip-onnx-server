...
async fn main() -> anyhow::Result<()> {
    ...
    info!("reading SigLIP ONNX service configuration");
    let service_config = ServiceConfig::from_environment()?;
    info!("loading local SigLIP ONNX text artifacts");
    ...
    info!(%socket_address, "binding SigLIP ONNX HTTP listener");
    let listener = tokio::net::TcpListener::bind(socket_address)
        .await
        .with_context(|| format!("binding SigLIP ONNX service on {socket_address}"))?;

    info!(
        model_id = service_config.model_id(),
        revision = service_config.model_revision(),
        dimension = service_config.embedding_dimension(),
        "starting CPU-only SigLIP ONNX text embedding service"
    );
